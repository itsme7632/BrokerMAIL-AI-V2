...
router.post("/admin/plans", requireAdmin, async (req, res): Promise<void> => {
  ...
  const [plan] = await db.insert(plansTable).values({
    ...
    buttonText,
    supportLevel,
    monthlyEmailLimit: parseInt(String(monthlyEmailLimit), 10),
    smtpAccountsLimit: parseInt(String(smtpAccountsLimit), 10),
    campaignsLimit: parseInt(String(campaignsLimit), 10),
    batchSendLimit: parseInt(String(batchSendLimit), 10),
  ...
  await db.insert(systemLogsTable).values({
    ...
    severity: "info",
    description: `Admin created plan "${name}" (${slug})`,
  });
...
router.put("/admin/plans/:id", requireAdmin, async (req, res): Promise<void> => {
  ...
  await db.insert(systemLogsTable).values({
    ...
    severity: "info",
    description: `Admin updated plan #${planId}`,
  });
