...
const SUB_TABS: { id: SubTab; label: string; icon: React.ElementType; group?: string }[] = [
  ...
  { id: "paymentMethods", label: "Payment Methods", icon: CreditCard,    group: "Users & Plans" },
...
export function AdminSettings() {
  ...
  const [payMethods, setPayMethods] = useState<{
    ...
    instructions: string | null; accountDetails: string | null;
    walletAddress: string | null; qrCodeUrl: string | null; sortOrder: number;
  }[]>([]);
  const [payMethodsLoading, setPayMethodsLoading] = useState(false);
  const [payMethodForm, setPayMethodForm] = useState({
    displayName: "", type: "paypal", isEnabled: true,
    instructions: "", accountDetails: "", walletAddress: "", qrCodeUrl: "", sortOrder: 0,
  });
  ...
  const [deletingPayMethod, setDeletingPayMethod] = useState<number | null>(null);
  ...
  useEffect(() => {
    if (activeTab === "paymentMethods") {
      ...
      apiFetch("payment-methods").then(setPayMethods).catch(() => {}).finally(() => setPayMethodsLoading(false));
    }
  }, [activeTab]);
      ...
      <div className="flex gap-5">
        ...
        <div className="flex-1 min-w-0 bg-white rounded-2xl border border-slate-200 shadow-sm p-6 overflow-hidden overflow-y-auto max-h-[calc(100vh-200px)]">
          ...
          {activeTab === "paymentMethods" && (
            <div className="space-y-5">
              <SectionHeader icon={CreditCard} title="Payment Methods" color="bg-blue-50 text-blue-600"
                desc="Manage the payment methods shown to users when they request a plan upgrade." />

              {/* Add new payment method form */}
              <div className="bg-slate-50 rounded-2xl border border-slate-100 p-4 space-y-4">
                <p className="text-sm font-semibold text-slate-800">{editingPayMethod ? "Edit Payment Method" : "Add Payment Method"}</p>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Display Name</label>
                    <input
                      value={payMethodForm.displayName}
                      onChange={e => setPayMethodForm(f => ({ ...f, displayName: e.target.value }))}
                      placeholder="e.g. PayPal, Bank Transfer, Wise"
                      className="w-full h-9 px-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Type</label>
                    <select
                      value={payMethodForm.type}
                      onChange={e => setPayMethodForm(f => ({ ...f, type: e.target.value }))}
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    onChange={e => setPayMethodForm(f => ({ ...f, instructions: e.target.value }))}
                    placeholder="Payment instructions shown to users (e.g. 'Send payment to the address below and include your reference code')"
                    rows={2}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Account Details / Email</label>
                    <input
                      value={payMethodForm.accountDetails}
                      onChange={e => setPayMethodForm(f => ({ ...f, accountDetails: e.target.value }))}
                      placeholder="payments@yourcompany.com"
                      className="w-full h-9 px-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Wallet Address (Crypto)</label>
                    <input
                      value={payMethodForm.walletAddress}
                      onChange={e => setPayMethodForm(f => ({ ...f, walletAddress: e.target.value }))}
                      placeholder="0x1234… or bc1q…"
                      className="w-full h-9 px-3 rounded-xl border border-slate-200 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">QR Code URL</label>
                    <input
                      value={payMethodForm.qrCodeUrl}
                      onChange={e => setPayMethodForm(f => ({ ...f, qrCodeUrl: e.target.value }))}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      type="number"
                      value={payMethodForm.sortOrder}
                      onChange={e => setPayMethodForm(f => ({ ...f, sortOrder: parseInt(e.target.value) || 0 }))}
                      className="w-full h-9 px-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <button
                    type="button"
                    onClick={() => setPayMethodForm(f => ({ ...f, isEnabled: !f.isEnabled }))}
                    className={`relative inline-flex h-6 w-11 flex-shrink-0 rounded-full border-2 border-transparent transition-colors focus:outline-none ${
                      payMethodForm.isEnabled ? "bg-blue-600" : "bg-slate-200"
                    }`}
                  >
                    <span className={`pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow ring-0 transition-transform ${payMethodForm.isEnabled ? "translate-x-5" : "translate-x-0"}`} />
                  </button>
                  <span className="text-sm text-slate-600">{payMethodForm.isEnabled ? "Enabled" : "Disabled"}</span>
                </div>
                <div className="flex gap-2 pt-1 border-t border-slate-100">
                  <Button
                    disabled={savingPayMethod || !payMethodForm.displayName}
                    onClick={async () => {
                      setSavingPayMethod(true);
                      try {
                        if (editingPayMethod) {
                          await apiFetch(`payment-methods/${editingPayMethod}`, { method: "PUT", body: JSON.stringify(payMethodForm) });
                          toast({ title: "Payment method updated" });
                        } else {
                          await apiFetch("payment-methods", { method: "POST", body: JSON.stringify(payMethodForm) });
                          toast({ title: "Payment method added" });
                        }
                        setEditingPayMethod(null);
                        setPayMethodForm({ displayName: "", type: "paypal", isEnabled: true, instructions: "", accountDetails: "", walletAddress: "", qrCodeUrl: "", sortOrder: 0 });
                  ...
                  >
                    {savingPayMethod ? <><Loader2 className="h-4 w-4 animate-spin" />Saving…</> : <><Save className="h-4 w-4" />{editingPayMethod ? "Update" : "Add Method"}</>}
                  </Button>
                  {editingPayMethod && (
                    <Button variant="outline" className="rounded-xl"
                      onClick={() => { setEditingPayMethod(null); setPayMethodForm({ displayName: "", type: "paypal", isEnabled: true, instructions: "", accountDetails: "", walletAddress: "", qrCodeUrl: "", sortOrder: 0 }); }}>
                      Cancel
                    </Button>
                  )}
                </div>
              ...
              {payMethodsLoading ? (
                <div className="space-y-2">{Array(3).fill(0).map((_, i) => <div key={i} className="h-16 bg-slate-100 rounded-xl animate-pulse" />)}</div>
              ) : payMethods.length === 0 ? (
                <div className="text-center py-10 text-slate-400">
                  <CreditCard className="h-8 w-8 mx-auto mb-2 opacity-40" />
                  <p className="text-sm">No payment methods added yet. Add one above to let users know how to pay.</p>
                </div>
              ) : (
                <div className="space-y-3">
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">{payMethods.length} method{payMethods.length !== 1 ? "s" : ""}</p>
                  {payMethods.map(m => (
                    <div key={m.id} className={`bg-white rounded-2xl border p-4 space-y-2 ${m.isEnabled ? "border-slate-200" : "border-slate-100 opacity-60"}`}>
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <p className="font-semibold text-slate-900 text-sm">{m.displayName}</p>
                            <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-slate-100 text-slate-600 capitalize">{m.type}</span>
                            {!m.isEnabled && <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-red-50 text-red-600">Disabled</span>}
                          </div>
                          {m.instructions && <p className="text-xs text-slate-500 mt-1">{m.instructions}</p>}
                          {m.accountDetails && <p className="text-xs font-mono text-slate-600 mt-0.5">{m.accountDetails}</p>}
                          {m.walletAddress && <p className="text-xs font-mono text-slate-500 mt-0.5 truncate">{m.walletAddress}</p>}
                        </div>
                        <div className="flex gap-2 flex-shrink-0">
                          <Button size="sm" variant="outline" className="h-7 text-xs rounded-lg px-2"
                            onClick={() => {
                              setEditingPayMethod(m.id);
                              setPayMethodForm({
                                displayName: m.displayName, type: m.type, isEnabled: m.isEnabled,
                                instructions: m.instructions ?? "", accountDetails: m.accountDetails ?? "",
                                walletAddress: m.walletAddress ?? "", qrCodeUrl: m.qrCodeUrl ?? "",
                          ...
                          <Button size="sm" variant="outline"
                            className="h-7 text-xs rounded-lg px-2 text-red-600 border-red-200 hover:bg-red-50"
                            disabled={deletingPayMethod === m.id}
                            onClick={async () => {
                              if (!confirm(`Delete "${m.displayName}"?`)) return;
                              setDeletingPayMethod(m.id);
                              try {
                                await apiFetch(`payment-methods/${m.id}`, { method: "DELETE" });
                                toast({ title: "Deleted" });
                                setPayMethods(prev => prev.filter(x => x.id !== m.id));
                              } catch (err: any) {
                                toast({ variant: "destructive", title: "Error", description: err.message });
                              } finally { setDeletingPayMethod(null); }
                            }}>
                            {deletingPayMethod === m.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <Trash2 className="h-3 w-3" />}
                          </Button>
                        </div>
                      </div>
                    </div>
